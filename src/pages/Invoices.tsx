...
export default function Invoices() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* Create / Edit Draft Invoice Dialog */}
      <Dialog open={showCreate} onOpenChange={(open) => { if (!open) { setShowCreate(false); resetForm(); } else setShowCreate(true); }}>
        <DialogContent className="w-[calc(100vw-2rem)] sm:max-w-2xl max-h-[90vh] p-0 flex flex-col gap-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label>Line Items ({newInv.items.length})</Label>
              ...
              <div className="rounded-md border border-border bg-muted/20 p-2 space-y-2">
                {newInv.items.map((item, idx) => {
                  ...
                  const lineTotal = qty * (prod?.selling_price || 0);
                  return (
                    <div key={idx} className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center bg-background rounded p-2 border border-border/60">
                      <div className="text-xs font-mono text-muted-foreground w-6 hidden sm:block">{idx + 1}.</div>
                      <ProductLineItemPicker
                        products={products}
                        value={item.product_id}
                        currency={currency}
                        onChange={(pid) => {
                          const items = [...newInv.items];
                          items[idx] = { ...items[idx], product_id: pid };
                      ...
                      <div className="flex items-center gap-2 sm:w-auto">
                        ...
                        />
                        <div className="text-xs text-right min-w-[80px] tabular-nums text-muted-foreground sm:text-foreground">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const items = newInv.items.filter((_, i) => i !== idx);
                            setNewInv({ ...newInv, items: items.length ? items : [{ product_id: "", qty: "1" }] });
                          }}
                        >
              ...
              <Button
                ...
                onClick={() => setNewInv({ ...newInv, items: [...newInv.items, { product_id: "", qty: "1" }] })}
              >
            ...
            {/* Running total */}
            {(() => {
              const subtotal = newInv.items.reduce((s, it) => {
                const prod = products.find(p => p.id === it.product_id);
                return s + ((parseInt(it.qty) || 0) * (prod?.selling_price || 0));
              }, 0);
              return (
                <div className="flex justify-end pt-2 border-t">
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground">Subtotal</p>
                    <p className="text-lg font-bold">{formatMoney(subtotal, currency)}</p>
                  </div>
                </div>
              );
            })()}
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 px-4 sm:px-6 py-3 border-t bg-background">
            <Button variant="outline" onClick={() => { setShowCreate(false); resetForm(); }} disabled={saving}>
              Cancel
            </Button>
            <Button variant="secondary" onClick={handleSaveDraft} disabled={saving} className="gap-1">
              <Save className="h-3.5 w-3.5" /> {saving ? "Saving..." : "Save Draft"}
            </Button>
            {!editingId && (
              <Button onClick={handleCreate} className="bg-accent text-accent-foreground hover:bg-accent/90" disabled={saving}>
