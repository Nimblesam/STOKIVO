...
import { Search, Building2, User, Eye, MessageSquare, Clock, AlertTriangle } from "lucide-react";
...
export default function AdminSupport() {
  ...
  const viewCompanyDetail = async (company: any) => {
    ...
    const [products, users, sales, invoices, sub] = await Promise.all([
      ...
      supabase.from("user_roles").select("id, role, user_id").eq("company_id", company.id),
  ...
  const createTicket = async () => {
    ...
    await supabase.from("admin_audit_logs").insert({
      ...
      metadata: { subject: ticketSubject, note: ticketNote, company: ticketCompany || undefined },
    } as any);
    await logAction("support_ticket_created", "support", undefined, { subject: ticketSubject, note: ticketNote });
          ...
          <Card>
            <CardContent className="pt-4">
              {supportNotes.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-6">No support notes yet</p>
              ) : (
                    ...
                    <div key={n.id} className="p-3 border rounded-lg">
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          {n.metadata?.subject && <Badge variant="outline" className="text-[10px]">{n.metadata.subject}</Badge>}
                        </div>
                      ...
                      <p className="text-sm">{n.metadata?.note || "—"}</p>
                      {n.metadata?.company && (
                        <p className="text-xs text-muted-foreground mt-1">Company: {n.metadata.company}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="border-t pt-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="text-center p-2 rounded-lg bg-muted/50">
                    <p className="text-lg font-bold">{companyDetail.users}</p>
